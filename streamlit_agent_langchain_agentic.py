...
from utils import create_dummy_agent, create_react_agent_with_memory
...
#     "complaints_engine",
#     "回答用户投诉相关问题，返回最相关的文档，如：建议增加人员删除功能，建议单位账号可以不使用管理员身份证号，可以自己设置，浮动公告飘的太快、遮挡信息，关闭按钮不明显，不方便关闭，客服联系方式遮挡信息，建议设置关闭按钮，查询统计的数据、怎么导出数据、怎么导出单位所有人的学习情况的数据，退休人员的账号怎么办、退休人员怎么调出本单位、怎么删除退休人员的账号，为什么不能手机网页登陆、手机网页不能登录、页面显示不全，建议添加新的专业的课程、课程里没有我的专业，有没有课件、没有课件讲解吗、有没有电子版讲义，课程不能倍速播放，视频播放太慢了、课程怎么快点播放、能快进吗，购买怎么不能一起支付、课程怎么一块买",
#     search_kwargs={"k": 10},
#     chunk_size=100,
#     separators=["\n\n"],
# )
...
#     "回答用户政策咨询相关问题，返回最相关的文档，如：职称评审什么时候、职称评审有什么要求，新一年继续教育学习时间、什么时候能报名学习、往年的课程还能补学吗、报名时间截止了，还能学吗、学习时间截止，还能开吗，报职称有什么要求，我需要继续教育吗、每年都需要继续教育吗，为什么要继续教育",
#     search_kwargs={"k": 5},
#     chunk_size=100,
#     separators=["\n\n"],
# )
...
# 系统操作咨询
individual_operation_agent_executor = create_react_agent_with_memory(
    tools=[individual_operation_tool]
)
employing_unit_operation_agent_executor = create_react_agent_with_memory(
    tools=[employing_unit_operation_tool]
)
supervisory_department_operation_agent_executor = create_react_agent_with_memory(
    tools=[supervisory_department_operation_tool]
)


def operation_check_user_role_and_route(info):
    print(info["topic"])
    if "unknown" in info["topic"]["output"].lower():
        return update_user_role_chain_executor
    elif "专技个人" in info["topic"]["output"].lower():
        return individual_operation_agent_executor
    elif "用人单位" in info["topic"]["output"].lower():
        return employing_unit_operation_agent_executor
    elif "主管部门" in info["topic"]["output"].lower():
        return supervisory_department_operation_agent_executor
    return main_qa_agent_executor
...
operation_chain = {
    "topic": check_user_role_router_chain_executor,
    "input": lambda x: x["input"],
} | RunnableLambda(operation_check_user_role_and_route)

# 修改信息咨询
personal_modify_info_agent_executor = create_react_agent_with_memory(
    tools=[personal_modify_info_tool]
)
employ_supervise_modify_info_agent_executor = create_react_agent_with_memory(
    tools=[employ_supervise_modify_info_tool]
)
cont_edu_modify_info_agent_executor = create_react_agent_with_memory(
    tools=[cont_edu_modify_info_tool]
)


def modify_info_check_user_role_and_route(info):
    print(info["topic"])
    if "unknown" in info["topic"]["output"].lower():
        return update_user_role_chain_executor
    elif "专技个人" in info["topic"]["output"].lower():
        return personal_modify_info_agent_executor
    elif "用人单位" in info["topic"]["output"].lower():
        return employ_supervise_modify_info_agent_executor
    elif "主管部门" in info["topic"]["output"].lower():
        return employ_supervise_modify_info_agent_executor
    return main_qa_agent_executor
...
modify_info_chain = {
    "topic": check_user_role_router_chain_executor,
    "input": lambda x: x["input"],
} | RunnableLambda(modify_info_check_user_role_and_route)
...
supervisory_department_registration_agent_executor = create_react_agent_with_memory(
    tools=[
        supervisory_department_registration_tool,
    ]
)
...
def registration_check_user_role_and_route(info):
    print(info["topic"])
    if "unknown" in info["topic"]["output"].lower():
        return update_user_role_chain_executor
    elif "专技个人" in info["topic"]["output"].lower():
        print("entering individual_registration_agent_executor")
        return individual_registration_agent_executor
    elif "用人单位" in info["topic"]["output"].lower():
        print("entering employing_unit_registration_agent_executor")
        return employing_unit_registration_agent_executor
    elif "主管部门" in info["topic"]["output"].lower():
        print("entering supervisory_department_registration_agent_executor")
        return supervisory_department_registration_agent_executor
    elif "继续教育机构" in info["topic"]["output"].lower():
        print("entering cont_edu_registration_agent_executor")
        return cont_edu_registration_agent_executor
    return main_qa_agent_executor
...
registration_chain = {
    "topic": check_user_role_router_chain_executor,
    "input": lambda x: x["input"],
} | RunnableLambda(registration_check_user_role_and_route)
...
# 查询注册信息
individual_check_registration_status_agent = create_react_agent_with_memory(
    tools=[RegistrationStatusToolIndividual()],
    prompt_str="""Your job is to use a tool to answer the following question.

        You MUST use a tool to answer the question. 
        If the user hasn't provided 身份证号，you MUST ask the user for that information.
        Try to answer the question as much as you can, but YOU MUST use a tool to answer the question.
        DO NOT answer the question without using a tool.
...
non_individual_check_registration_status_agent = create_react_agent_with_memory(
    tools=[RegistrationStatusToolNonIndividual()],
    prompt_str="""Your ONLY job is to use a tool to answer the following question.

        You MUST use a tool to answer the question. 
        If the user hasn't provided 管理员身份证号、单位名称、统一信用代码，you MUST ask the user for that information.
        Try to answer the question as much as you can, but YOU MUST use a tool to answer the question.
        DO NOT answer the question without using a tool.
...
def query_registration_status_route(info):
    print(info["topic"])
    if "unknown" in info["topic"]["output"].lower():
        return update_user_role_chain_executor
    elif "专技个人" in info["topic"]["output"].lower():
        return individual_check_registration_status_agent
    elif "用人单位" in info["topic"]["output"].lower():
        return non_individual_check_registration_status_agent
    elif "主管部门" in info["topic"]["output"].lower():
        return non_individual_check_registration_status_agent
    elif "继续教育机构" in info["topic"]["output"].lower():
        return non_individual_check_registration_status_agent
...
check_registration_status_chain = {
    "topic": check_user_role_router_chain_executor,
    "input": lambda x: x["input"],
} | RunnableLambda(query_registration_status_route)

# 其他
other_questions_agent_executor = create_react_agent_with_memory(
    # tools=[complaints_tool, policy_inquiry_tool, other_questions_tool]
    tools=[all_other_questions_tool]
)
...
template = """Given the user input AND chat history below, classify whether the conversation topic or user mentioned being about `学时没显示` or `学时有问题` or `学时申报` or `学时审核` or `济宁市：如何报班、报名` or `济宁市：课程进度` or `济宁市：多个设备，其他地方登录` or `济宁市：课程退款退费，课程买错了` or `济宁市：课程找不到，课程没有了` or `单位调转` or `人员变更` or `人员信息` or `修改信息` or `上级部门是谁` or `注册咨询` or `注册问题` or `账号审核` or `登录账号信息查询` or `无法登录` or `忘记密码` or `找回密码` or `查询注册状态` or `查询管理员` or `有没有注册` or `济宁市` or `退休人员` or `other`.

# Do not answer the question. Simply classify it as being related to `学时没显示` or `学时有问题` or `学时申报` or `学时审核` or `济宁市：如何报班、报名` or `济宁市：课程进度` or `济宁市：多个设备，其他地方登录` or `济宁市：课程退款退费，课程买错了` or `济宁市：课程找不到，课程没有了` or `单位调转` or `人员变更` or `人员信息` or `修改信息` or `上级部门是谁` or `注册咨询` or `注册问题` or `账号审核` or `登录账号信息查询` or `无法登录` or `忘记密码` or `找回密码` or `查询注册状态` or `查询管理员` or `有没有注册` or `济宁市` or `退休人员` or `other`.
# Do not respond with anything other than `学时没显示` or `学时有问题` or `学时申报` or `学时审核` or `济宁市：如何报班、报名` or `济宁市：课程进度` or `济宁市：多个设备，其他地方登录` or `济宁市：课程退款退费，课程买错了` or `济宁市：课程找不到，课程没有了` or `单位调转` or `人员变更` or `人员信息` or `修改信息` or `上级部门是谁` or `注册咨询` or `注册问题` or `账号审核` or `登录账号信息查询` or `无法登录` or `忘记密码` or `查询注册状态` or `查询管理员` or `有没有注册` or `济宁市` or `退休人员` or `other`.
